...
MUInt32 TsStream::mpegts_read_header()
{
	...
	if (!file.Open("playlist-00002.ts", mv3File::stream_read))
	{
		return -1;
	}
...
MUInt32 TsStream::parse_ts(MByte* buffer_packet)
{
	...
	if (tsHeader.pid == SDT_PID)
	{
		//printf("SDT \n");
		
	}
	else if(tsHeader.pid == PAT_PID)
	{
		//printf("PAT \n");
		pat_cb(buffer_packet_section_strat_pos);
	}
	else if (tsHeader.pid == PMT_PID)
	{
		//printf("PMT \n");
...
MUInt32 TsStream::pat_cb(MByte* section_start_pos)
{
	...
	MInt32 sid = 0;
	MInt32 pmt_pid = 0;
	MByte* section_sid = section_start_pos + 8;
	...
	for(MUInt32 i = 0;i < program_length ;i++)
	{
		...
		pmt_pid = to_UInt16(section_sid +2);
		if (pmt_pid < 0)
			break;
		pmt_pid &= 0x1fff;
		section_sid = section_sid + 4;
	}
}
...
MUInt32 TsStream::pmt_cb(MByte* section_start_pos)
{
	...
	m_pcr_pid = to_UInt16(section_start_pos + 8);
	if (m_pcr_pid < 0)
		return 0;
	m_pcr_pid &= 0x1fff;

	MUInt16 program_info_length = to_UInt16(section_start_pos + 10);
	if (program_info_length < 0)
		return -1;
	program_info_length &= 0xfff;

	//27 0x1b H264
	m_streamVideo.stream_type = section_start_pos[12];
	m_streamVideo.pid = to_UInt16(section_start_pos+13) & 0x1fff;
	MUInt16  desc_list_len  = to_UInt16(section_start_pos + 15);
	if (desc_list_len < 0)
		return -1;
	desc_list_len &= 0xfff;
...
MInt32 TsStream::parse_section_header(MByte* biffer_section_header,SectionHeader &section_header)
{
	...
	val = biffer_section_header[0];
	if (val < 0)
		return -1;

	section_header.tid = val;

	section_header.section_length = to_UInt16(biffer_section_header + 1) & 0xFFF;

	val = to_UInt16(biffer_section_header+3);
	if (val < 0)
		return val;

	section_header.id = val;

	val = biffer_section_header[5];
	if (val < 0)
		return val;
	section_header.version = (val >> 1) & 0x1f;
	val = biffer_section_header[6];
	if (val < 0)
		return val;
	section_header.sec_num = val;
	val = biffer_section_header[7];
	if (val < 0)
		return val;

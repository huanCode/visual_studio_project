...
MRESULT     MEventMultipleWait(MLong lEventCnt, MHandle* phEventArray, MBool bWaitAll, MDWord dwTimerOut, MBool bAlertable);
...
MRESULT		MThreadDestroy(MHandle hThread);	//end a thread, called by parent thread
MVoid		MThreadExit();	//exit current calling thread
...
MRESULT 	MThreadDetach(MHandle hThread);	//indicates that the child thread will release resources by itself and then exit
